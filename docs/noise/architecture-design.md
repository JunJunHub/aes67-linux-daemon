...
| Sink 删除 / PTP 失锁 | flush 对应 processor 的插件状态（停止 process、丢弃内部状态），见 §10 风险 9 |
...
class NoiseManager {
 public:
  ...
  // PTP 失锁联动：flush 所有 processor 的有状态插件
  void on_ptp_unlocked();

 private:
  struct SensorContext {
    uint8_t sink_id;
    std::unique_ptr<NoiseDetector> detector;
    std::unique_ptr<NoiseAnalyzer> analyzer;
    std::unique_ptr<DenoiseProcessor> denoise;  // 各自持有自己的 IDenoisePlugin
    std::unique_ptr<NoiseMetrics> metrics;
  };
  NoiseAudioBridge& bridge_;
  std::map<uint8_t, SensorContext> sensors_;  // sensor_id -> per-sensor 处理组件
  std::mutex ctrl_mutex_;  // 保护 sensors_ 增删（控制线程）；帧回调走 RCU/双缓冲读，避免阻塞音频线程
};
```

> **帧回调线程安全**：`on_frame` 在 Bridge 的 capture 线程高频调用，不能持 `ctrl_mutex_` 阻塞。传感器增删是低频控制线程操作，用 RCU 式双缓冲或 `shared_ptr<const SensorTable>` 原子交换让音频线程无锁快照读，增删瞬间新老表短暂共存。与 §4.2 的准热切换 `std::atomic<shared_ptr>` 同一思路。
...
private:
  ...
  std::map<uint8_t, SinkEntry> sink_entries_;
...
| 11 | `std::atomic<shared_ptr>` 在 C++17 下非无锁 | `std::atomic_load/store` 废弃自由函数内部用自旋锁，实时音频线程调用可能优先级反转 | 实现时用 RcuPtr（双 shared_ptr + atomic index）或细粒度读写锁替代，§4.2 已标注 |
