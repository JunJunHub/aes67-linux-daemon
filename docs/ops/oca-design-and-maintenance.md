...
flowchart TB
    ...
    subgraph L2["L2 对象模型"]
        OBJ["Object/Registry"]
        SES["Session"]
        CLS["15 个对象类<br/>Root/Agent/Worker/Manager/Block<br/>DevMgr/NetMgr/SubMgr/MCM<br/>AppNet/Network/CtrlNet<br/>MC3/MTN/MTN_AES67<br/>(Spec4: emitter 注入)"]
    end
    subgraph L1["L1 编解码"]
...
| OcaMediaTransportNetwork（抽象） | - | {1,4,2} | 1 | - | Spec5 |
| OcaMediaTransportNetworkAES67 | 8192 | {1,4,2,0xFFFF,0xFA,0x2EE9,1} | 1 | MediaTransportNetworkAES67 | Spec5 |
...
#### OcaMediaClockManager（DefLevel 3，ONo 7）

| 方法 | 索引 | 必选 | 行为 |
|------|------|------|------|
| GetClocks | 1 | ✅ | 返空列表（废弃 Clock，用 Clock3） |
| GetMediaClockTypesSupported | 2 | ✅ | 返空 List<OcaMediaClockType>（设备用 Clock3） |
| GetClock3s | 3 | ✅ | 返 [8193] |

#### OcaMediaClock3（DefLevel 3，ONo 8193）
...
#### OcaMediaTransportNetwork（DefLevel 3，抽象基类）
...
#### OcaMediaTransportNetworkAES67（DefLevel 7，ONo 8192）

ClassID {1,4,2,0xFFFF,0xFA,0x2EE9,1}（7 字段，defLevel=7）。Fitcan 控制器按此 ClassID 搜索（GetMembersRecursive 后比对 ClassID）。
...
```bash
./tests/oca-test -p          # 全量（39 源用例；WITH_AVAHI=OFF 时 38,mdns_publisher_smoke 编译出）
./tests/oca-test -p -t oca_e2e_acceptance   # 单跑 E2E
```

39 个用例分布（WITH_AVAHI=ON 全 39；OFF 时 38,mdns_publisher_smoke 编译出）：
...
| L2 单测 | registry_find_and_range、session_subscription_and_queue、session_keepalive_expiry、dispatch_root_block、dispatch_root_lock_unlock、dispatch_device_manager、dispatch_device_manager_2018_methods、dispatch_device_manager_2023_methods、dispatch_network_manager、dispatch_subscription_ev2、dispatch_subscription_ev1_ev2_propertychange、dispatch_cm3_network_objects、dispatch_agent_methods、dispatch_appnet_methods、dispatch_worker_label_owner |
| L2 单测(Spec4) | dispatch_property_changed_label_emit、dispatch_property_changed_enabled_emit |
| L2 单测(Spec5) | dispatch_media_clock_manager、dispatch_media_clock3_methods、dispatch_media_clock3_rate_via_bridge、dispatch_mtn_aes67_readonly、dispatch_mtn_connector_crud_via_bridge |
| L3 集成 | transport_keepalive_and_command、oca_server_facade |
| 回归 | transport_empty_body_probe_keeps_connection、transport_rejects_oversized_pdu（畸形 PDU 回归）、mdns_publisher_smoke（AVAHI 守卫） |
| E2E 验收 | **oca_e2e_acceptance**（Spec1 端到端）、oca_e2e_property_changed（Spec4 PropertyChanged 端到端） |
...
本节系统记录对接 Fitcan AES70Controller 过程中发现的**与标准 AES70 不一致或 Fitcan 私有**的协议接口，以及 daemon 的兼容实现方式。原则：**优先兼容两种标准（AES70-2018 合规工具 + Fitcan 控制器），用最小修改达成双通**。

### 权威源原则

Fitcan 私有协议的权威源是 Fitcan 项目的 `OcaLiteLib` + `AES70DeviceManager` 源码（`/home/Share/Project/FCPro/MFitcanManager/vsxprojs/`），**不是标准 AES70 文档**。遇到 Fitcan 行为与标准不符时，以 OcaLiteLib 源码为准。记忆：[[ocamicro-connector-format-source]]。

> **自测盲点警示**：daemon 与 oca-probe 共享同一套编解码常量，自测（oca-probe / oca-test）能过不代表 Fitcan 控制器能过--两者在 NrParameters 语义、GetMembers 格式、TXT key 名、连接器格式上有多处分歧，只有真实 Fitcan 控制器 / Aes70CompliancyTestTool 能暴露。

### 兼容性差异总表

| # | 接口/字段 | 标准 AES70 | Fitcan 私有/分歧 | 影响 | 兼容实现 | 最简修改建议 |
|---|----------|-----------|-----------------|------|---------|------------|
| 1 | mDNS TXT key 名 | 无强制约定（合规工具只要求 `txtvers`+`protovers`） | `IP_P`/`MAC_P`/`IP_S`/`MAC_S`/`ServicePortNo`/`DeviceID`/`SubChNum`/`FirmwareVersion` | Fitcan 硬过滤 `IP_P==0 && IP_S==0` 丢弃设备 | daemon 发 Fitcan 私有名，`IP_P` 填真实 IP | 已修复，见下方「mDNS TXT」 |
| 2 | 连接器线缆格式 | 规范含 connectorID/pinMap/connectorStatus | connectorID=u16（非u32）、pinMap项=`{u16,u8,u16}`（无ownerONo）、connectorStatus缺errorCode、GetPorts mode 1/2（非0/1） | Fitcan 控制器无法解析 connector | 按 OCAMicro `OcaLiteMediaSourceConnector` Marshal 重写 | 已修复（C2），见 Spec5 审查修复 |
| 3 | OcaMediaClockTypesSupported | AES70-2018 Mandatory，应返 List | 原返 NotImplemented(8) | 合规工具 test4/test5 失败（mandatory 方法 result!=8 才过） | 返空 List | `rsp.u16(0); return {OK,1};` |
| 4 | OcaMediaClock(8194) 存根 | 已废弃类 | 合规工具仍检查其 mandatory 方法，NI 不过 | test5 失败 | 移除存根（不注册） | 删类文件+CMake+methods 常量 |
| 5 | ClassID Manager 类 | OCAMicro `{1,3,x}` | ocac-2018 用 `{1,3,x}`；2023 PDF 误读为 `{1,2,x}` | 工具误识别 DeviceManager 为 OcaNetwork | 用 `{1,3,x}`（OCAMicro 编译期常量为权威） | Spec2 已修复（G0） |
| 6 | NrParameters 字段 | 规范=参数计数（u8） | 早期 daemon=参数字节长度 | Fitcan 控制器解析响应错乱 | 改为参数计数，paramBytes 由 commandSize/responseSize 推导 | Spec1 已修复（ef19171） |
| 7 | GetMembers 返回 | `List<ObjectIdentification>`（每元素 ONo+ClassID+ClassVersion） | 早期 daemon 返裸 ONo 列表 | 控制器结构漂移解析失败 | 写完整 ObjectIdentification | Spec1 已修复（db371a2） |
| 8 | OcaApplicationNetwork defLevel | =classID.fieldCount=2 | 早期误认=3 | AppNet 方法分派不到 | defLevel=2 分派 | Spec2 已修复（Fix-D） |
| 9 | EV1 AddSubscription | 2018 工具用 EV1(mi=1, 5参数) | daemon 原只有 EV2(mi=8) | PropertyChanged 订阅失败 | 补 EV1 mi=1/2/5/6，响应 0 参数 | Spec2 已修复（T1） |
| 10 | 空体探测命令 | 返 BadFormat | 早期整体断连 | 单命令异常断整个连接 | 单命令 try/catch 返 BadFormat(3) | Spec2 已修复（5ca5a74） |
| 11 | MTN_AES67 ClassID | 厂商扩展 `{1,4,2,0xFFFF,0xFA,0x2EE9,1}` | Fitcan 控制器按此 ClassID 搜索 | 控制器发现不到媒体网络 | 7 字段 ClassID，defLevel=7 | 已实现 |
| 12 | GetIDAdvertised | 应返 node_id | 早期返 ip_addr | 设备标识错 | 返 `bridge->get_device_id()`=node_id | 已修复（I1） |

### mDNS TXT key 名（本节核心，最新修复）

Fitcan 控制器发现设备的链路：mDNS 浏览 `_oca._tcp` -> 解析 TXT（`GetInfoFromTXT_Record`）-> 硬过滤 -> A/SRV 记录解析 -> TCP 连接。

**硬过滤**（`AES70Manager_ThreadOnBrowsingDevice.cpp:147`）：
```cpp
if ((DeviceQuery.nTxtRecordIp[0] == 0) && (DeviceQuery.nTxtRecordIp[1] == 0)) continue;
```
`IP_P` 与 `IP_S` 同时为 0 即丢弃。daemon 早期发标准风格 `ip_addr`（Fitcan 解析为 0）-> 命中硬过滤 -> 设备不可见。Aes70CompliancyTestTool 只要求 `txtvers`+`protovers`，故合规通过而 Fitcan 不可见（自测盲点）。

**故障连接检查**（`AES70Manager_ThreadOnBrowsingDevice.cpp:100-130`）：A 记录解析的 IP 必须 == TXT `IP_P`，否则判「故障连接」跳过。故 `IP_P` 必填**真实 IP**（与 A 记录一致）。

**控制连接端口**用 **SRV 记录端口**（`AES70Browser_OnQueryReply.cpp:260`），非 TXT 的 `ServicePortNo`。daemon 的 SRV 端口 = `transport_->port()` = 65037，正确。TXT 的 `ServicePortNo` 仅信息字段。

daemon 实际发布的 TXT（`mdns_publisher.cpp`）：

| TXT key | 值 | 必需性 | 说明 |
|---------|-----|-------|------|
| `IP_P` | 真实 IP（如 172.16.1.198） | **必需** | 解除硬过滤；须与 A 记录一致 |
| `MAC_P` | 真实 MAC | 建议 | 设备信息 |
| `ServicePortNo` | 65037 | 建议 | 与 SRV 一致；非连接端口 |
| `DeviceID` | 0 | 可选 | Fitcan 按 `atol` 数字解析，非字符串；非过滤 |
| `SubChNum` | 1 | 可选 | 每路流子通道数（1=mono），非总通道数 |
| `FirmwareVersion` | daemon_version | 可选 | 设备信息 |
| `txtvers`/`protovers` | 1 | - | 合规工具要求 |

> `IP_S`/`MAC_S`（ST-2022-7 备接口）单接口设备不发；硬过滤是 `&&`，`IP_P` 有值即过。Fitcan 自有设备单接口时发 `IP_S=0.0.0.0`/`MAC_S=000000000000`。

### 兼容实现方式总结

1. **协议层分歧（#5-#11）**：统一以 OCAMicro 编译期常量 + sphinx 2024 RST 为权威源修正，使 daemon 同时满足 2018 合规工具与 Fitcan 控制器。authority 链：OCAMicro 编译期常量 > sphinx 2024 > 2018 工具 ref table > PDF 文本 > memory。
2. **私有扩展（#1/#2/#11/#12）**：Fitcan 私有字段（TXT key、连接器格式、厂商 ClassID）在 daemon 侧用 Fitcan 私有名实现，不污染标准协议层（`daemon/oca/` 零核心依赖）。标准合规工具读不到这些私有字段但不报错（非强制）。
3. **强制方法不返 NotImplemented（#3/#4）**：mandatory 方法必须 `result != 8`。返空 List / 合理默认值即可过，NI 必失败。废弃类不要注册（注册即被测）。

### 遇到新兼容性问题的最简修改决策

1. **先定位权威源**：grep OcaLiteLib / AES70DeviceManager 源码确认 Fitcan 期望的确切字段名/格式/值，不猜。
2. **优先双通**：能同时满足合规工具与 Fitcan 的实现优先（如 mandatory 方法返值而非 NI）。
3. **私有扩展隔离**：Fitcan 私有字段集中在 `mdns_publisher.cpp`（TXT）/`media_transport_network*.cpp`（连接器），不扩散到标准对象类。
4. **废弃类不注册**：AES70 已弃用的对象类（如 OcaMediaClock{1,2,6}）不要注册到 OcaServer，注册即被合规工具检查 mandatory 方法。用替代类（OcaMediaClock3{1,2,15}）。
5. **真机验证**：oca-probe/oca-test 是自测盲点，协议层分歧必须用 Aes70CompliancyTestTool + Fitcan 控制器真机验证。

## Spec5：媒体桥接

Spec5 实现总体架构中「运行时桥接（OcaAudioBridge）」层的具体接口与消费它的 OCA 对象，把 daemon 从"协议合规自包含门面"升级为"可经 AES70 控制器真实操控音频流"的设备：

- **3 个媒体类对象**注册到 OcaServer：OcaMediaClockManager{1,3,7}@7、OcaMediaClock3{1,2,15}@8193、OcaMediaTransportNetworkAES67{1,4,2,0xFFFF,0xFA,0x2EE9,1}@8192（含 MTN{1,4,2} 基类）。OcaMediaClock{1,2,6} 废弃类不注册（见下「Spec5 合规回归」注）。
...
Spec5 初版（`d61385f`..`d3a2a77`）经 gstack-review 发现 7 个真实缺陷，均不在原 38/38 测试覆盖内，已修复（`1eb6b5b`..`fa33b5a`，oca-test 增至 39/39,移除 OcaMediaClock 废弃存根测试后）：
...
Spec5 媒体类（OcaMediaClockManager / OcaMediaClock3 / OcaMediaTransportNetwork / OcaMediaTransportNetworkAES67）的已实现方法表已统一收录到「对象模型（L2）> 各对象已实现方法」，与其它对象并列以便单一查阅。本节仅保留 Spec5 阶段叙事（审查修复、Connector 映射、Network 现实化、mDNS TXT）。
...
`_oca._tcp` 的 TXT 记录 key 名遵循 **Fitcan 控制器私有约定**，非 AES70 标准风格。权威源：Fitcan `OcaLiteLib` 的 `AES70Browser_OnResolveServiceName.cpp::GetInfoFromTXT_Record`（TXT 解析）+ `AES70Manager_ThreadOnBrowsingDevice.cpp:147`（硬过滤）。

> **Fitcan 硬过滤**：若 TXT 中 `IP_P` 与 `IP_S` 同时为 0，设备被直接丢弃（`continue`，不进设备列表）。单接口设备必须发非空 `IP_P`（且须与 mDNS A 记录解析的实际 IP 一致，否则「故障连接检查」`AES70Manager_ThreadOnBrowsingDevice.cpp:100-130` 亦会跳过）。service type `_oca._tcp` 本身是标准的（故 Aes70CompliancyTestTool 与 avahi 都能发现 daemon），Fitcan 发现失败的根因在 TXT key 名不匹配 + 硬过滤命中，而非 service type。

daemon 实际发布的 TXT（`mdns_publisher.cpp`，Fitcan 私有名）：

| TXT key | 值来源 | 必需性 | Fitcan 用途 |
|---------|--------|--------|------------|
| `IP_P` | `cfg_.ip_addr`（真实 IP） | **必需** | 硬过滤 + 故障连接检查；与 A 记录一致 |
| `MAC_P` | `cfg_.mac_addr`（真实 MAC） | 建议 | 设备信息 |
| `ServicePortNo` | OCP.1 端口（65037） | 建议 | 仅信息；控制连接实际用 **SRV 记录端口** |
| `DeviceID` | 0 | 可选 | Fitcan 按 `atol` 十进制数字解析；非过滤/非连接依赖 |
| `SubChNum` | 1 | 可选 | 每路流子通道数（1=mono），非总通道数；默认 1 |
| `FirmwareVersion` | `cfg_.daemon_version` | 可选 | 设备信息 |
| `txtvers`/`protovers` | 1 | - | Aes70CompliancyTestTool 要求（≥2 条 TXT） |

> `IP_S`/`MAC_S`（备接口，ST-2022-7 冗余）单接口设备不发；硬过滤是 `&&`，`IP_P` 有值即过。

`MdnsTxtRecords` 结构体（`mdns_publisher.hpp`）：ip_addr / ip_addr_sec / mac_addr / firmware。由 `OcaServerConfig` 扩展字段（main.cpp 从 Config 填充）传入，`MdnsPublisher` 构造时接收。控制连接端口取自 `Transport` 的 SRV 端口（`transport_->port()`），非 TXT。
...
| Spec5 | 媒体桥接(Bridge+MTN+Clock+mDNS) | 34->39 | ✅ 完成 |
