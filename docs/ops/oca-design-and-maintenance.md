...
flowchart TB
    ...
    subgraph L2["L2 对象模型"]
        OBJ["Object/Registry"]
        SES["Session"]
        CLS["9 个对象类<br/>Root/Agent/Worker/Block<br/>DevMgr/NetMgr/SubMgr<br/>AppNet/Network/CtrlNet"]
    end
    subgraph L1["L1 编解码"]
...
| L2 对象 | `classes/{root,agent,device_manager,network_manager,subscription_manager,application_network,network,control_network}.{hpp,cpp}` | OcaRoot/Agent/Worker/Manager/Block/AppNet/Network 层次 + 9 个具体对象 |
...
| GetLabel | 8 | 返 role() 字符串 | Spec3 |
| SetLabel | 9 | 读可选 OcaString，no-op OK | Spec3 |
...
- 验证基线：oca-test 31/31（Spec3 终态）、Aes70CompliancyTestTool v2.0.1 AES70-2018 **5/5**（阶段三终态）
