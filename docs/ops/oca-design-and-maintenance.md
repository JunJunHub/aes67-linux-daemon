...
**关键解耦**：`OcaServer` 依赖 POD `OcaServerConfig`（由 main.cpp 从 `Config` 填充）+ `OcaAudioBridge*`（纯虚接口），不直接依赖 `Config`/`SessionManager`。因此 `oca-test` 无需链接 `config.cpp`/`json.cpp`/`session_manager.cpp`，OCA 栈可独立测试。`OcaSessionManagerBridge`（bridge 实现）仅在 main.cpp 链接的 daemon 目标中。
...
```mermaid
flowchart TB
    OCA["OCA 对象<br/>(MTN/MC3/Network)"]
    BR["OcaAudioBridge<br/>(纯虚接口)"]
    SMB["OcaSessionManagerBridge<br/>(实现)"]
    SM["SessionManager<br/>+ Config + DriverManager"]
    OCA --> BR
    BR --> SMB
    SMB --> SM
```

OCA 编译单元零依赖 SessionManager/Config/DriverManager 头文件。接口从 OCA 需求倒推，不暴露 `StreamSource`/`StreamSink` 等内部类型。唯一实现 `OcaSessionManagerBridge` 在 `.cpp` 中 include 完整头文件做类型转换。

Bridge 接口分组：PTP 时钟（get_ptp_config/set_ptp_config/get_ptp_status）、采样率（get/set_sample_rate, get_supported_sample_rates）、Source CRUD、Sink CRUD、网络信息、IO 端口数、Observer 回调。
...
- 验证基线：oca-test 38/38（Spec5 终态）、Aes70CompliancyTestTool v2.0.1 AES70-2018 **5/5**（阶段三终态）
