...
Bridge 接口分组：PTP 时钟（get_ptp_config/set_ptp_config/get_ptp_status）、采样率（get/set_sample_rate, get_supported_sample_rates）、Source CRUD、Sink CRUD、网络信息（含 get_device_id 返回 node_id）、IO 端口数、Observer 回调。
