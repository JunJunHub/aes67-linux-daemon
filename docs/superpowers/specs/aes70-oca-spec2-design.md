...
2. 据确认结果拆分 `docs/superpowers/plans/aes70-oca-spec2-phase1-plan.md`:阶段一(有证据项)立即可实施,阶段二标注前置条件。
3. 阶段一实施 -> oca-test + MITM + 2018 工具验证 -> 阶段二待文本源。

## Spec3:CM3 对象补齐(冲 5/5,AES70-2018 合规最小强制实例)

> 计划:`docs/superpowers/plans/aes70-oca-spec2-phase3-plan.md`。延续 `feature/aes70-oca`。
> 状态(2026-07-11):Fix-A(GetMembersRecursive)+ Fix-B(OcaNetwork{1,2,1} / OcaControlNetwork{1,4,1} 实例化)已实装,oca-test 28/28、oca-probe 阶段三全 OK;真机 test4 验收待用户 Win 重跑。

### test4 失败根因(源码精读坐实,非 classID 编码问题)

工具 `MinimumObjectCompliancyTest::GetObjects`(MinimumObjectCompliancyTest.cpp:246-291)逻辑:
1. line 252 `GetClassIdentification(OCA_ROOT_BLOCK_ONO=100)` OK → line 255 `outputMembers.Add(根块{1,1,3}@100)`。
2. line 260 `GetMembersRecursive(100)`:daemon 原 `kBlockGetMembersRecursive`(defLevel3 mi=6)返 **NotImplemented(6)** → `bTestRootBlockResult=false`。
3. 走 else 回退(line 272-287):line 276 `GetMembers(100)` 返 [1,4,6],line 279 **`outputMembers = members;`(赋值覆盖)** → **根块 100 丢失**,只剩 [1,4,6]。
4. `HandleMembers` 仅对 classID 是 OcaBlock{1,1,3} 前缀的成员递归;管理器{1,3,*} 非前缀 → 不递归。
5. 最终 `deviceReportedObjects`=[1,4,6],根块缺席 → OcaBlock{1.1.3} `bFound=false` → "Missing OcaBlock";OcaNetwork/OcaControlNetwork 未实例化 → 同 Missing。`mandatoryObjects=0`。

**匹配逻辑本身(line 537-543)无 bug**:OcaBlock{1,1,3} vs 根块{1,1,3} fieldCount 均 3、memcmp==0 本应匹配。问题纯是根块没进 deviceReportedObjects。

范围收敛:日志只报 3 条 Missing(OcaBlock/OcaNetwork/OcaControlNetwork),**未报 OcaMediaTransportNetwork**——其 mandatory 受 `DeviceType=Streaming` 门控(BaseTestClass.cpp:499-504),用户跑工具时未传 `-t streaming` → 不满足 → isMandatory 留 false。OcaApplicationNetwork 无 MandatoryMap=true。故 CM3 实例化收敛到 **2 类**。

### daemon 侧解法

- **Fix-A:实现 `OcaBlock::GetMembersRecursive`(defLevel3 mi=6)** 返 OK + Ocp1List<OcaBlockMember>(每元素 = {ONo u32 + ClassID + ClassVersion u16 + ContainerONo u32})。工具 GetObjects 即走 line 262 if 累加分支(不覆盖根块)→ deviceReportedObjects 含根块 100 → OcaBlock 匹配。GetMembers 同步扩展含 CM3 成员(objects_in_range(1,99) + (4096,65535))。
- **Fix-B:实例化 OcaNetwork{1,2,1} @ONo 4097 + OcaControlNetwork{1,4,1} @ONo 4098**,注册 registry,由 Fix-A 的 GetMembers/Recursive 自动纳入成员列表。defLevel == classID.fieldCount == 3(OCAMicro 全代码库 `if (methodID.GetDefLevel() == CLASS_ID.GetFieldCount())` 模式核对)。

### 2023 弃用立场(用户拍板)

用户明确"符合 AES70-2018,但标记 2023 已弃用"(非"启用",经确认——AES70-2023 实际把 OcaNetwork 移出最小集,应弃用;2018 先例:OcaNetwork 即 DeprecatedSince 2018 仍 mandatory)。源码注释(`network.hpp/.cpp`)与设计文档显式声明:**本实例仅为兼容 AES70-2018 合规工具的最小强制实例,2023 已弃用,fork 专有不回上游**。

### CM3 对象 2018 强制方法(最小合规返值)

| 类 | ONo | ClassID | 强制方法(2018) | daemon 返值 |
|----|-----|---------|----------------|-------------|
| OcaNetwork | 4097 | {1,2,1} v1 | GetLinkType(1) | u8 EthernetWired=1 |
| | | | GetIDAdvertised(2) | 空 OcaBlob(u16=0) |
| | | | GetControlProtocol(4) | u8 OCP.1=1 |
| | | | GetMediaProtocol(5) | u8 None=0 |
| | | | GetSystemInterfaces(9) | 空 List(u16=0) |
| OcaControlNetwork | 4098 | {1,4,1} v1 | GetControlProtocol(1) | u8 OCP.1=1 |

CheckMethods 判据(MinimumObjectCompliancyTest.cpp:55-57):mandatory 方法 status 非 BadMethod/BadONo/NotImplemented 即"implements"。本表全返 OK,合规。

### oca-test / oca-probe 验证(Spec3 实装后)

- oca-test 28/28:新增 `dispatch_cm3_network_objects`(classID 编码 + 2018 强制方法返值 + OcaRoot 基类委托);`dispatch_root_block` 增 GetMembersRecursive;`oca_server_facade`/`oca_e2e_acceptance` 成员数 3→5 并校验 2 CM3 条目。
- oca-probe 阶段三全 OK:GetMembersRecursive 返 5 成员(含 ContainerONo=100)+ OcaNetwork 5 方法 + OcaControlNetwork 1 方法全返 OK。
