#!/bin/bash
#
# noise-daemonctl.sh - AES67 daemon 真实硬件控制脚本(供 noise-dev.sh run-real 委托)
#
# 本分支 feature/noise 暂不接入 OCA/AES70 控制协议,仅 HTTP 控制平面。
# --oca 选项保留但本分支不生效(daemon 无 OCA 代码,配置中 oca_enabled 会被忽略)。
#
# Usage:
#   ./noise-daemonctl.sh start   [-i <iface>] [--ptp-iface <iface>] [--no-ptp]
#                          [-p <port>] [-d <ptp_domain>] [-a <addr>] [--unload-on-stop]
#                          [--daemon-bin <path>]
#   ./noise-daemonctl.sh stop    [--unload]
#   ./noise-daemonctl.sh restart [-i <iface>] [--ptp-iface <iface>] [--no-ptp] ...
#   ./noise-daemonctl.sh status
#
# Options:
...
start() {
  ...
  if [ "$NO_PTP" = 1 ]; then
    ...
    if is_running "$(ptp_pidfile)"; then
      ...
      warn "    1) ./noise-daemonctl.sh start -i $IFACE --ptp-iface lo   # run ptp4l on loopback"
      warn "    2) ./noise-daemonctl.sh start -i $IFACE --no-ptp         # skip ptp4l (browse only, no audio)"
      rm -f "$(ptp_pidfile)"
  ...
  log "Stop:       ./noise-daemonctl.sh stop"
}
...
Options:
  ...
  --oca           (本分支不生效) feature/noise 无 OCA 代码,传入仅往配置写一个被
                  daemon 忽略的 oca_enabled 字段。保留选项仅为与 oca-daemonctl.sh
                  保持参数兼容。
  --daemon-bin <path>  use a specific daemon binary (overrides --oca auto-select).
                  Point at the noise build from './noise-dev.sh build --real'.
